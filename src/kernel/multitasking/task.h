...
#define TASK_FLAG_SYSTEM    (1 << 2)
#define TASK_FLAG_RING      0b11
...
#define TaskCodeSegment(__task)         (((__task).flags & TASK_FLAG_RING) == 0b11 ? USER_CODE_SEGMENT | 0b11 : KERNEL_CODE_SEGMENT)
#define TaskDataSegment(__task)         (((__task).flags & TASK_FLAG_RING) == 0b11 ? USER_DATA_SEGMENT | 0b11 : KERNEL_DATA_SEGMENT)
#define TaskPagingPrivilege(__task)     (((__task).flags & TASK_FLAG_RING) == 0b11 ? PAGING_USER_LEVEL : PAGING_SUPERVISOR_LEVEL)

struct Task CreateTask(char* name, virtual_address_t address, uint8_t ring, bool system);
